...
coin_list = ['BTC-USD','ETH','HOT1','RSR','NULS','NIM','AION','QASH','VITE','AAPL','QRL','BCN','GBYTE','LBC','POA','PAC','ILC','BEPRO','GO','XMC']
...
data = yf.download(crypto_currency, start=start, end=end, progress=False,)
...
def input_set(dt0,dt1):
    ...
    data11 = yf.download(crypto_currency, start=dt00, end=dt1, progress=False,)
    ...
    y_test11 = []
    c = look_back
    for x11 in range(look_back,len(test_scaled111)-future_day+2):                             #   (60, 397))      (60,27+60-1+1)=(60,87)
    ...
    # for x11 in range(look_back,look_back+delta+1):                             #   (60, 397))      (60,27+60-1+1)=(60,87)
    #     x_test11.append(test_scaled111[x11-look_back:x11, 0])
    #     y_test11.append(test_scaled111[x11:x11+future_day, 0])
    #     c +=1
    #     print(f'printing c :{c}')
    # print(np.array(x_test11).shape)
    # print(y_test11)
    # print(30*'--')
    # print(x_test11)
    # print(np.array(y_test11).shape)
    #x_test11, y_test11 = np.array(x_test11), np.array(y_test11)                             #   x_train : (483, 60), y_train :   (483,1)
    x_test11 = np.array(x_test11)
    print(y_test11)
    y_test111 = np.empty((27, 1))
    #y_test111 = np.array(y_test11)
    print(y_test11[0],y_test111[0])
    for i in range(27):
        y_test111[i,1] = y_test11[i]
    ...
    print(y_test11)
    print(30*'--')
    print(x_test11.shape,y_test11.shape,type(x_test11),type(y_test11))
